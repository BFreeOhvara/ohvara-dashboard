/**
 * CloserModal — the closer's call-prep popup.
 *
 * Uses CallPrepModal (same shared box as CallModal) so the chrome is
 * structurally identical. Only the data and save logic differ.
 *
 * Props:
 *   appt    — appointment object (with appt.lead)
...
export function CloserModal({ appt, onClose }) {
  ...
  async function handleComplete() {
    ...
    try {
      ...
      if (outcome === 'closed') {
        ...
        }).catch(err => console.error('[CloserModal] create-commission-payout failed:', err))
      }
      ...
      if (outcome === 'lost' && appt.demo_client_id) {
        ...
        try {
          ...
          console.error('[CloserModal] cleanup-lost-demo failed:', err)
        } finally {
